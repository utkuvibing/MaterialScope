...
TRANSLATIONS = {
    ...
    "dash.home.prereq_no_datasets_body": {
        "tr": "Henüz veri seti yok. Çalışma alanını doldurmak için ölçüm dosyalarınızı yükleyin.",
        "en": "No datasets are loaded yet. Upload your measurement files to populate this workspace.",
    },
    ...
    "dash.home.stepper.step2_desc": {"tr": "Ölçüm dosyalarınızı yükleyin", "en": "Upload your measurement files"},
    ...
    "dash.analysis.ftir.axis_conversion": {"tr": "Eksen dönüşümü: {detail}", "en": "Axis conversion: {detail}"},
    "dash.analysis.ftir.signal_conversion": {"tr": "Sinyal dönüşümü: {detail}", "en": "Signal conversion: {detail}"},
    "dash.analysis.ftir.region_integration": {"tr": "Bölge entegrasyonu: {detail}", "en": "Region integration: {detail}"},
    ...
    "dash.analysis.raman.depth.title": {"tr": "Spektral Derinlik", "en": "Spectral Depth"},
    "dash.analysis.raman.depth.hint": {"tr": "Eksen dönüşümü (dalga boyu ↔ Raman kayması, uyarım dalga boyu gerektirir) ve bölge entegrasyonu.", "en": "Axis conversion (wavelength ↔ Raman shift, requires an excitation wavelength) and region integration."},
    "dash.analysis.raman.depth.axis_target": {"tr": "Eksen hedefi", "en": "Axis target"},
    "dash.analysis.raman.depth.axis_target.as_is": {"tr": "Olduğu gibi", "en": "As imported"},
    "dash.analysis.raman.depth.axis_target.raman_shift": {"tr": "Raman kayması (cm⁻¹)", "en": "Raman shift (cm⁻¹)"},
    "dash.analysis.raman.depth.axis_target.wavenumber": {"tr": "Dalga sayısı (cm⁻¹)", "en": "Wavenumber (cm⁻¹)"},
    "dash.analysis.raman.depth.axis_target.wavelength_nm": {"tr": "Dalga boyu (nm)", "en": "Wavelength (nm)"},
    "dash.analysis.raman.depth.axis_source": {"tr": "Kaynak eksen birimi", "en": "Source axis unit"},
    "dash.analysis.raman.depth.axis_source.auto": {"tr": "Veri setinden (bildirilen)", "en": "From dataset (declared)"},
    "dash.analysis.raman.depth.axis_source.raman_shift": {"tr": "Raman kayması (cm⁻¹)", "en": "Raman shift (cm⁻¹)"},
    "dash.analysis.raman.depth.axis_source.cm1": {"tr": "cm⁻¹ (mutlak)", "en": "cm⁻¹ (absolute)"},
    "dash.analysis.raman.depth.axis_source.nm": {"tr": "nm", "en": "nm"},
    "dash.analysis.raman.depth.laser_wavelength": {"tr": "Uyarım dalga boyu (nm)", "en": "Excitation wavelength (nm)"},
    "dash.analysis.raman.depth.regions_label": {"tr": "Entegrasyon bölgeleri", "en": "Integration regions"},
    "dash.analysis.raman.depth.regions_hint": {"tr": "Satır başına bir bölge: 'alt üst etiket' (eksen birimleri).", "en": "One region per line: 'low high label' (axis units)."},
    "dash.analysis.raman.depth.summary_axis": {"tr": "Eksen", "en": "Effective axis"},
    "dash.analysis.raman.depth.summary_regions": {"tr": "Bölge alanları", "en": "Region areas"},
    "dash.analysis.raman.axis_conversion": {"tr": "Eksen dönüşümü: {detail}", "en": "Axis conversion: {detail}"},
    "dash.analysis.raman.region_integration": {"tr": "Bölge entegrasyonu: {detail}", "en": "Region integration: {detail}"},
    ...
    "dash.analysis.xrd.raw_quality.card_title": {"tr": "Ham Veri Kalitesi", "en": "Raw Data Quality"},
    "dash.analysis.xrd.raw_quality.card_hint": {"tr": "Çalıştırmadan önce veri setinin sağlamlığını değerlendirin.", "en": "Assess dataset sanity before running analysis."},
    "dash.analysis.xrd.raw_quality.pick_dataset": {"tr": "Kalite bilgisi için bir veri seti seçin.", "en": "Select a dataset to see quality information."},
    "dash.analysis.xrd.raw_quality.load_failed": {"tr": "Kalite yüklenemedi: {error}", "en": "Failed to load quality: {error}"},
    "dash.analysis.xrd.raw_quality.empty_missing_series": {"tr": "Veri yok veya eksen/sinyal kolonları çıkarılamadı.", "en": "No data or axis/signal columns could not be extracted."},
    "dash.analysis.xrd.raw_quality.empty_too_few_points": {"tr": "Kalite bilgisi için yeterli veri noktası yok.", "en": "Not enough data points for quality information."},
    "dash.analysis.xrd.raw_quality.stat_points": {"tr": "Nokta sayısı: {n:,}", "en": "Points: {n:,}"},
    "dash.analysis.xrd.raw_quality.stat_axis_range": {"tr": "Eksen aralığı: {a0:.3f} – {a1:.3f} {u}", "en": "Axis range: {a0:.3f} – {a1:.3f} {u}"},
    "dash.analysis.xrd.raw_quality.stat_signal_range": {"tr": "Sinyal aralığı: {s0:.4f} – {s1:.4f} {u}", "en": "Signal range: {s0:.4f} – {s1:.4f} {u}"},
    "dash.analysis.xrd.raw_quality.stat_missing": {"tr": "Eksik/geçersiz nokta: {n}", "en": "Missing/invalid points: {n}"},
    "dash.analysis.xrd.raw_quality.stat_spacing_cv": {"tr": "Aralık düzensizliği (CV): {cv}", "en": "Spacing irregularity (CV): {cv}"},
    "dash.analysis.xrd.raw_quality.stat_edge_drift": {"tr": "Kenar kayması: {drift}", "en": "Edge drift: {drift}"},
    "dash.analysis.xrd.raw_quality.hint.axis_non_monotonic": {"tr": "Eksen tek yönlü değil; sıralamayı gözden geçirin.", "en": "Axis is not monotonic; review ordering."},
    "dash.analysis.xrd.raw_quality.hint.axis_mostly_decreasing": {"tr": "Eksen azalan sırada; XRD için artan eksen beklenir.", "en": "Axis is decreasing; increasing order is expected for XRD."},
    "dash.analysis.xrd.raw_quality.hint.irregular_spacing": {"tr": "Aralık düzensizliği yüksek; örnekleme tutarsızlığı olabilir.", "en": "High spacing irregularity; sampling inconsistency possible."},
    "dash.analysis.xrd.raw_quality.hint.somewhat_irregular_spacing": {"tr": "Aralık düzensizliği hafif.", "en": "Slight spacing irregularity."},
    "dash.analysis.xrd.raw_quality.hint.strong_edge_drift": {"tr": "Güçlü kenar kayması var; baseline ayarlarını kontrol edin.", "en": "Strong edge drift; check baseline settings."},
    "dash.analysis.xrd.raw_quality.hint.moderate_edge_drift": {"tr": "Orta düzey kenar kayması var.", "en": "Moderate edge drift."},
    ...
    "dash.analysis.xrd.baseline.hint": {"tr": "Yuvarlanan minimum, doğrusal veya AsLS.", "en": "Rolling minimum, linear, or AsLS."},
    ...
    "dash.analysis.xrd.baseline.asls": {"tr": "AsLS (asimetrik en küçük kareler)", "en": "AsLS (asymmetric least squares)"},
    "dash.analysis.xrd.baseline.lam": {"tr": "λ (yumuşaklık)", "en": "λ (smoothness)"},
    "dash.analysis.xrd.baseline.asymmetry": {"tr": "p (asimetri)", "en": "p (asymmetry)"},
    "dash.analysis.xrd.scherrer.card_title": {"tr": "Scherrer kristalit boyutu", "en": "Scherrer crystallite size"},
    "dash.analysis.xrd.scherrer.hint": {
        "tr": "İsteğe bağlı Scherrer tahmini; dalga boyu ve varsayımlar kaydedilir.",
        "en": "Optional Scherrer estimate; wavelength and assumptions are recorded.",
    },
    "dash.analysis.xrd.scherrer.enabled": {"tr": "Scherrer hesapla", "en": "Compute Scherrer"},
    "dash.analysis.xrd.scherrer.shape_factor": {"tr": "Şekil faktörü K", "en": "Shape factor K"},
    "dash.analysis.xrd.scherrer.instrumental_fwhm": {
        "tr": "Cihaz FWHM (2θ°, boş = düzeltme yok)",
        "en": "Instrumental FWHM (2θ°, blank = no correction)",
    },
    "dash.analysis.xrd.matching_blocked": {
        "tr": "Eşleştirme engellendi: {reason}",
        "en": "Matching blocked: {reason}",
    },
    "dash.analysis.xrd.wavelength_gate_line": {
        "tr": "Dalga boyu süzgeci: {candidates} aday, {peaks} tepe hariç tutuldu.",
        "en": "Wavelength gate: {candidates} candidates, {peaks} peaks excluded.",
    },
    "dash.analysis.xrd.scherrer.summary_line": {
        "tr": "Scherrer: {count} tepe hesaplandı, medyan {median} (K={k}, λ={wavelength} Å).",
        "en": "Scherrer: {count} peaks computed, median {median} (K={k}, lambda={wavelength} angstrom).",
    },
    "dash.analysis.xrd.scherrer.assumptions_line": {
        "tr": "FWHM birimi: {fwhm}; cihaz genişlemesi: {instrumental}.",
        "en": "FWHM units: {fwhm}; instrumental broadening: {instrumental}.",
    },
    "dash.analysis.xrd.scherrer.withheld_line": {
        "tr": "Scherrer tutuldu: {reason}",
        "en": "Scherrer withheld: {reason}",
    },
    ...
    "dash.analysis.dsc.integration.bound_max": {"tr": "Üst sınır (°C)", "en": "Upper bound (°C)"},
    "dash.analysis.dsc.integration.snap": {"tr": "Karakterize onset/endset'e yasla", "en": "Snap to characterized onset/endset"},
    "dash.analysis.dsc.integration.apply_btn": {"tr": "Entegrasyonu uygula", "en": "Apply Integration"},
    "dash.analysis.dsc.integration.help.enable": {
        "tr": "Açıkken tespit edilen tepeler yapılandırılan sınırlar üzerinden yeniden entegre edilir; boş sınırlar karakterize onset/endset kullanır.",
        "en": "When enabled, detected peaks are re-integrated over configured bounds; empty bounds use each peak's characterized onset/endset.",
    },
    "dash.analysis.dsc.integration.help.bound_min": {
        "tr": "Entegrasyon penceresinin alt sıcaklık sınırı. Boş bırakılırsa her tepe kendi onset/endset'i üzerinden entegre edilir.",
        "en": "Lower temperature bound of the integration window. Leave empty to integrate each peak over its onset/endset.",
    },
    "dash.analysis.dsc.integration.help.bound_max": {
        "tr": "Entegrasyon penceresinin üst sıcaklık sınırı.",
        "en": "Upper temperature bound of the integration window.",
    },
    "dash.analysis.dsc.integration.help.snap": {
        "tr": "Açıkken girilen pencere tepeyi seçer ve gerçek entegrasyon sınırları karakterize onset/endset'e yaslanır.",
        "en": "When enabled, the typed window selects the peak and the actual integration bounds snap to its characterized onset/endset.",
    },
    "dash.analysis.dsc.integration.applied": {"tr": "Uygulandı", "en": "Applied"},
    "dash.analysis.dsc.integration.status_disabled": {"tr": "Varsayılan FWHM penceresi", "en": "Default FWHM window"},
    "dash.analysis.dsc.integration.bounds_custom": {"tr": "pencere [{tmin}, {tmax}]", "en": "window [{tmin}, {tmax}]"},
    "dash.analysis.dsc.integration.bounds_characterized": {"tr": "karakterize onset/endset", "en": "characterized onset/endset"},
    "dash.analysis.dsc.integration.snap_suffix": {"tr": "yaslanmış", "en": "snapped"},
    "dash.analysis.dsc.integration.bounds_line": {"tr": "Entegrasyon sınırları", "en": "Integration bounds"},
    "dash.analysis.dsc.integration.mode.characterized": {"tr": "karakterize sınırlar", "en": "characterized bounds"},
    "dash.analysis.dsc.integration.mode.custom": {"tr": "özel pencere", "en": "custom window"},
    "dash.analysis.dsc.integration.mode.custom_snapped": {"tr": "özel pencere → onset/endset", "en": "custom window → onset/endset"},
    "dash.analysis.dsc.integration.mode.fwhm_window": {"tr": "FWHM penceresi", "en": "FWHM window"},
    ...
    "dash.analysis.tga.raw_quality.card_title": {"tr": "Ham Veri Kalitesi", "en": "Raw Data Quality"},
    "dash.analysis.tga.raw_quality.card_hint": {
        "tr": "Çalıştırmadan önce veri setinin sağlamlığını değerlendirin.",
        "en": "Assess dataset sanity before running analysis.",
    },
    ...
    "dash.analysis.ftir.depth.title": {"tr": "Spektral Derinlik", "en": "Spectral Depth"},
    "dash.analysis.ftir.depth.hint": {"tr": "Eksen dönüşümü, %T→A dönüşümü ve bölge (bant alanı) entegrasyonu.", "en": "Axis conversion, %T→A conversion, and region (band-area) integration."},
    "dash.analysis.ftir.depth.axis_target": {"tr": "Eksen hedefi", "en": "Axis target"},
    "dash.analysis.ftir.depth.axis_target.as_is": {"tr": "Olduğu gibi", "en": "As imported"},
    "dash.analysis.ftir.depth.axis_target.wavenumber": {"tr": "Dalga sayısı (cm⁻¹)", "en": "Wavenumber (cm⁻¹)"},
    "dash.analysis.ftir.depth.axis_target.wavelength_um": {"tr": "Dalga boyu (µm)", "en": "Wavelength (µm)"},
    "dash.analysis.ftir.depth.axis_target.wavelength_nm": {"tr": "Dalga boyu (nm)", "en": "Wavelength (nm)"},
    "dash.analysis.ftir.depth.axis_source": {"tr": "Kaynak eksen birimi", "en": "Source axis unit"},
    "dash.analysis.ftir.depth.axis_source.auto": {"tr": "Veri setinden (bildirilen)", "en": "From dataset (declared)"},
    "dash.analysis.ftir.depth.axis_source.cm1": {"tr": "cm⁻¹", "en": "cm⁻¹"},
    "dash.analysis.ftir.depth.axis_source.um": {"tr": "µm", "en": "µm"},
    "dash.analysis.ftir.depth.axis_source.nm": {"tr": "nm", "en": "nm"},
    "dash.analysis.ftir.depth.absorbance_toggle": {"tr": "Geçirgenliği absorbansa çevir (A = −log₁₀ T)", "en": "Convert transmittance to absorbance (A = −log₁₀ T)"},
    "dash.analysis.ftir.depth.regions_label": {"tr": "Entegrasyon bölgeleri", "en": "Integration regions"},
    "dash.analysis.ftir.depth.regions_hint": {"tr": "Satır başına bir bölge: 'alt üst etiket' (eksen birimleri).", "en": "One region per line: 'low high label' (axis units)."},
    "dash.analysis.ftir.depth.summary_axis": {"tr": "Eksen", "en": "Effective axis"},
    "dash.analysis.ftir.depth.summary_signal_basis": {"tr": "Sinyal temeli", "en": "Signal basis"},
    "dash.analysis.ftir.depth.summary_absorbance": {"tr": "Absorbans (%T'den dönüştürüldü)", "en": "Absorbance (converted from %T)"},
    "dash.analysis.ftir.depth.summary_regions": {"tr": "Bölge alanları", "en": "Region areas"},
    ...
    "dash.analysis.dsc.raw_quality.card_title": {"tr": "Ham Veri Kalitesi", "en": "Raw Data Quality"},
    "dash.analysis.dsc.raw_quality.card_hint": {"tr": "Çalıştırmadan önce veri setinin sağlamlığını değerlendirin.", "en": "Assess dataset sanity before running analysis."},
    "dash.analysis.dsc.raw_quality.pick_dataset": {"tr": "Kalite bilgisi için bir veri seti seçin.", "en": "Select a dataset to see quality information."},
    "dash.analysis.dsc.raw_quality.load_failed": {"tr": "Kalite yüklenemedi: {error}", "en": "Failed to load quality: {error}"},
    "dash.analysis.dsc.raw_quality.empty_missing_series": {"tr": "Veri yok veya sıcaklık/sinyal kolonları çıkarılamadı.", "en": "No data or temperature/signal columns could not be extracted."},
    "dash.analysis.dsc.raw_quality.empty_too_few_points": {"tr": "Kalite bilgisi için yeterli veri noktası yok.", "en": "Not enough data points for quality information."},
    "dash.analysis.dsc.raw_quality.stat_points": {"tr": "Nokta sayısı: {n:,}", "en": "Points: {n:,}"},
    "dash.analysis.dsc.raw_quality.stat_axis_range": {"tr": "Sıcaklık aralığı: {a0:.1f} – {a1:.1f} {u}", "en": "Temperature range: {a0:.1f} – {a1:.1f} {u}"},
    "dash.analysis.dsc.raw_quality.stat_signal_range": {"tr": "Sinyal aralığı: {s0:.4f} – {s1:.4f} {u}", "en": "Signal range: {s0:.4f} – {s1:.4f} {u}"},
    "dash.analysis.dsc.raw_quality.stat_missing": {"tr": "Eksik/geçersiz nokta: {n}", "en": "Missing/invalid points: {n}"},
    "dash.analysis.dsc.raw_quality.stat_spacing_cv": {"tr": "Aralık düzensizliği (CV): {cv}", "en": "Spacing irregularity (CV): {cv}"},
    "dash.analysis.dsc.raw_quality.stat_edge_drift": {"tr": "Kenar kayması: {drift}", "en": "Edge drift: {drift}"},
    "dash.analysis.dsc.raw_quality.hint.axis_non_monotonic": {"tr": "Sıcaklık ekseni tek yönlü değil; sıralamayı gözden geçirin.", "en": "Temperature axis is not monotonic; review ordering."},
    "dash.analysis.dsc.raw_quality.hint.axis_mostly_decreasing": {"tr": "Sıcaklık ekseni azalan sırada.", "en": "Temperature axis is decreasing."},
    "dash.analysis.dsc.raw_quality.hint.irregular_spacing": {"tr": "Aralık düzensizliği yüksek; örnekleme tutarsızlığı olabilir.", "en": "High spacing irregularity; sampling inconsistency possible."},
    "dash.analysis.dsc.raw_quality.hint.somewhat_irregular_spacing": {"tr": "Aralık düzensizliği hafif.", "en": "Slight spacing irregularity."},
    "dash.analysis.dsc.raw_quality.hint.strong_edge_drift": {"tr": "Güçlü kenar kayması var; baseline ayarlarını kontrol edin.", "en": "Strong edge drift; check baseline settings."},
    "dash.analysis.dsc.raw_quality.hint.moderate_edge_drift": {"tr": "Orta düzey kenar kayması var.", "en": "Moderate edge drift."},
    ...
    "dash.analysis.dta.raw_quality.card_title": {"tr": "Ham Veri Kalitesi", "en": "Raw Data Quality"},
    "dash.analysis.dta.raw_quality.card_hint": {"tr": "Çalıştırmadan önce veri setinin sağlamlığını değerlendirin.", "en": "Assess dataset sanity before running analysis."},
    "dash.analysis.dta.raw_quality.pick_dataset": {"tr": "Kalite bilgisi için bir veri seti seçin.", "en": "Select a dataset to see quality information."},
    "dash.analysis.dta.raw_quality.load_failed": {"tr": "Kalite yüklenemedi: {error}", "en": "Failed to load quality: {error}"},
    "dash.analysis.dta.raw_quality.empty_missing_series": {"tr": "Veri yok veya sıcaklık/sinyal kolonları çıkarılamadı.", "en": "No data or temperature/signal columns could not be extracted."},
    "dash.analysis.dta.raw_quality.empty_too_few_points": {"tr": "Kalite bilgisi için yeterli veri noktası yok.", "en": "Not enough data points for quality information."},
    "dash.analysis.dta.raw_quality.stat_points": {"tr": "Nokta sayısı: {n:,}", "en": "Points: {n:,}"},
    "dash.analysis.dta.raw_quality.stat_axis_range": {"tr": "Sıcaklık aralığı: {a0:.1f} – {a1:.1f} {u}", "en": "Temperature range: {a0:.1f} – {a1:.1f} {u}"},
    "dash.analysis.dta.raw_quality.stat_signal_range": {"tr": "Sinyal aralığı: {s0:.4f} – {s1:.4f} {u}", "en": "Signal range: {s0:.4f} – {s1:.4f} {u}"},
    "dash.analysis.dta.raw_quality.stat_missing": {"tr": "Eksik/geçersiz nokta: {n}", "en": "Missing/invalid points: {n}"},
    "dash.analysis.dta.raw_quality.stat_spacing_cv": {"tr": "Aralık düzensizliği (CV): {cv}", "en": "Spacing irregularity (CV): {cv}"},
    "dash.analysis.dta.raw_quality.stat_edge_drift": {"tr": "Kenar kayması: {drift}", "en": "Edge drift: {drift}"},
    "dash.analysis.dta.raw_quality.hint.axis_non_monotonic": {"tr": "Sıcaklık ekseni tek yönlü değil; sıralamayı gözden geçirin.", "en": "Temperature axis is not monotonic; review ordering."},
    "dash.analysis.dta.raw_quality.hint.axis_mostly_decreasing": {"tr": "Sıcaklık ekseni azalan sırada.", "en": "Temperature axis is decreasing."},
    "dash.analysis.dta.raw_quality.hint.irregular_spacing": {"tr": "Aralık düzensizliği yüksek; örnekleme tutarsızlığı olabilir.", "en": "High spacing irregularity; sampling inconsistency possible."},
    "dash.analysis.dta.raw_quality.hint.somewhat_irregular_spacing": {"tr": "Aralık düzensizliği hafif.", "en": "Slight spacing irregularity."},
    ...
    "dash.about.header.title": {"tr": "MaterialScope Hakkında", "en": "About MaterialScope"},
    "dash.about.header.subtitle": {
        "tr": "Ürün bilgisi, yetenekler, mimari ve geliştirme durumu.",
        "en": "Product information, capabilities, architecture, and development status.",
    },
    "dash.about.header.badge": {"tr": "ÜRÜN BİLGİSİ", "en": "PRODUCT INFO"},
    "dash.about.what.title": {"tr": "MaterialScope nedir?", "en": "What is MaterialScope?"},
    "dash.about.what.copy_1": {
        "tr": "MaterialScope; QC, Ar-Ge ve akademik araştırma iş akışları için tasarlanmış, cihaz üreticisinden bağımsız, çok modlu bir malzeme karakterizasyon çalışma alanıdır.",
        "en": "MaterialScope is a vendor-independent, multimodal materials characterization workbench designed for QC, R&D, and academic research workflows.",
    },
    "dash.about.what.copy_2": {
        "tr": "Platform DSC, TGA, DTA, FTIR, Raman ve XRD verilerini tek bir birleşik çalışma alanına getirir; kullanıcıların cihaz yazılımları, elektronik tablolar, çizim araçları ve ayrı rapor belgeleri arasında sürekli geçiş yapmadan karakterizasyon sonuçlarını içe aktarmasına, görselleştirmesine, karşılaştırmasına, analiz etmesine ve raporlamaya hazırlamasına yardımcı olur.",
        "en": "The platform brings DSC, TGA, DTA, FTIR, Raman, and XRD data into a single unified workspace, helping users import, visualize, compare, analyze, and report characterization results without constantly switching between vendor software, spreadsheets, plotting tools, and separate report documents.",
    },
    "dash.about.audience.title": {"tr": "Kimler İçin", "en": "Who It Is For"},
    "dash.about.audience.research": {"tr": "Araştırma ekipleri", "en": "Research teams"},
    "dash.about.audience.research_title": {"tr": "Malzeme Bilimi ve Mühendisliği araştırmacıları", "en": "Materials Science & Engineering researchers"},
    "dash.about.audience.research_copy": {
        "tr": "Termal, spektroskopik ve difraksiyon kanıtlarını izlenebilir sonuçlarla birleştirin.",
        "en": "Unify thermal, spectroscopic, and diffraction evidence while keeping results traceable.",
    },
    "dash.about.audience.qc": {"tr": "QC / Ar-Ge laboratuvarları", "en": "QC / R&D labs"},
    "dash.about.audience.qc_title": {"tr": "Kalite kontrol ve geliştirme laboratuvarları", "en": "Quality control and development laboratories"},
    "dash.about.audience.qc_copy": {
        "tr": "Rutin koşuları karşılaştırın, figürleri yakalayın ve tutarlı karakterizasyon çıktıları hazırlayın.",
        "en": "Compare routine runs, capture figures, and prepare consistent characterization outputs.",
    },
    "dash.about.audience.graduate": {"tr": "Lisansüstü iş akışları", "en": "Graduate workflows"},
    "dash.about.audience.graduate_title": {"tr": "Lisansüstü öğrenciler ve tez projeleri", "en": "Graduate students and thesis projects"},
    "dash.about.audience.graduate_copy": {
        "tr": "İçe aktarılan cihaz dosyalarından yorumlanabilir görsellere ve rapora hazır notlara ilerleyin.",
        "en": "Move from imported instrument files to interpretable visuals and report-ready notes.",
    },
    "dash.about.audience.testing": {"tr": "Test ekipleri", "en": "Testing groups"},
    "dash.about.audience.testing_title": {"tr": "Endüstriyel malzeme test ekipleri", "en": "Industrial material testing teams"},
    "dash.about.audience.testing_copy": {
        "tr": "DSC, TGA, DTA, FTIR, Raman ve XRD veri setlerinde tekrarlı numune incelemesini destekleyin.",
        "en": "Support repeated sample review across DSC, TGA, DTA, FTIR, Raman, and XRD datasets.",
    },
    "dash.about.audience.partners": {"tr": "Tasarım ortakları", "en": "Design partners"},
    "dash.about.audience.partners_title": {"tr": "Çok teknikli karakterizasyon projeleri", "en": "Multi-technique characterization projects"},
    "dash.about.audience.partners_copy": {
        "tr": "Gelişen iş akışlarını değerlendirin; doğrulama, raporlama ve entegrasyon önceliklerini şekillendirmeye yardımcı olun.",
        "en": "Evaluate emerging workflows and help shape validation, reporting, and integration priorities.",
    },
    "dash.about.capability.title": {"tr": "Temel Yetenekler", "en": "Core Capabilities"},
    "dash.about.capability.import": {"tr": "Çok modlu karakterizasyon verisi içe aktarma", "en": "Multimodal characterization data import"},
    "dash.about.capability.visualization": {"tr": "Plotly tabanlı etkileşimli bilimsel görselleştirme", "en": "Interactive Plotly-based scientific visualization"},
    "dash.about.capability.analysis": {"tr": "DSC, TGA, DTA, FTIR, Raman ve XRD analiz iş akışı desteği", "en": "DSC, TGA, DTA, FTIR, Raman, and XRD analysis workflow support"},
    "dash.about.capability.comparison": {"tr": "Paylaşılan çalışma alanında numune/koşu karşılaştırması", "en": "Sample/run comparison in a shared workspace"},
    "dash.about.capability.workspace": {"tr": "Proje düzeyinde organizasyon ve yakın analiz geçmişi", "en": "Project-level organization and recent analysis history"},
    "dash.about.capability.figures": {"tr": "Figür yakalama ve rapor odaklı çıktı hazırlığı", "en": "Figure capture and report-oriented output preparation"},
    "dash.about.capability.reporting": {"tr": "Yapılandırılmış bilimsel raporlama desteği", "en": "Structured scientific reporting support"},
    "dash.about.modalities.title": {"tr": "Desteklenen Modaliteler", "en": "Supported Modalities"},
    "dash.about.modalities.modality": {"tr": "Modalite", "en": "Modality"},
    "dash.about.modalities.full_name": {"tr": "Tam Ad", "en": "Full Name"},
    "dash.about.modalities.current_scope": {"tr": "Mevcut Kapsam", "en": "Current Scope"},
    "dash.about.modalities.dsc_scope": {"tr": "Termal olay görselleştirme, eğri inceleme ve karşılaştırma iş akışı desteği.", "en": "Thermal event visualization, curve inspection, and comparison workflow support."},
    "dash.about.modalities.tga_scope": {"tr": "Kütle kaybı eğrisi görselleştirme, bozunma davranışı inceleme ve karşılaştırmalı termal kararlılık iş akışları.", "en": "Mass-loss curve visualization, degradation behavior inspection, and comparative thermal stability workflows."},
    "dash.about.modalities.dta_scope": {"tr": "Diferansiyel termal yanıt görselleştirme ve karşılaştırma desteği.", "en": "Differential thermal response visualization and comparison support."},
    "dash.about.modalities.ftir_scope": {"tr": "Spektrum görselleştirme, pik inceleme ve fonksiyonel grup odaklı yorumlama desteği.", "en": "Spectrum visualization, peak inspection, and functional-group-oriented interpretation support."},
    "dash.about.modalities.raman_scope": {"tr": "Raman spektrumu görselleştirme, karşılaştırma ve pik odaklı inceleme desteği.", "en": "Raman spectrum visualization, comparison, and peak-oriented inspection support."},
    "dash.about.modalities.xrd_scope": {"tr": "Difraksiyon deseni görselleştirme, pik inceleme ve faz analizi odaklı iş akışı desteği.", "en": "Diffraction pattern visualization, peak inspection, and phase-analysis-oriented workflow support."},
    "dash.about.workflow.title": {"tr": "Tipik İş Akışı", "en": "Typical Workflow"},
    "dash.about.workflow.import": {"tr": "Karakterizasyon verisini içe aktar", "en": "Import characterization data"},
    "dash.about.workflow.select": {"tr": "İlgili analiz modülünü seç", "en": "Select the relevant analysis module"},
    "dash.about.workflow.inspect": {"tr": "Eğrileri, spektrumları veya difraksiyon desenlerini incele", "en": "Inspect curves, spectra, or diffraction patterns"},
    "dash.about.workflow.compare": {"tr": "Koşuları veya numuneleri karşılaştır", "en": "Compare runs or samples"},
    "dash.about.workflow.capture": {"tr": "Figürleri ve notları yakala", "en": "Capture figures and notes"},
    "dash.about.workflow.report": {"tr": "Yapılandırılmış rapor çıktıları hazırla", "en": "Prepare structured report outputs"},
    "dash.about.architecture.title": {"tr": "Mimari", "en": "Architecture"},
    "dash.about.architecture.copy": {
        "tr": "MaterialScope, görselleştirme, backend servisleri ve analiz mantığını ayırmak için tasarlanmış modüler bir bilimsel yazılım mimarisi kullanır.",
        "en": "MaterialScope uses a modular scientific software architecture designed to separate visualization, backend services, and analysis logic.",
    },
    "dash.about.architecture.frontend": {"tr": "Etkileşimli bilimsel görselleştirme için Dash + Plotly frontend", "en": "Dash + Plotly frontend for interactive scientific visualization"},
    "dash.about.architecture.backend": {"tr": "Servis ayrımı ve gelecekte ölçeklenebilirlik için FastAPI backend", "en": "FastAPI backend for service separation and future scalability"},
    "dash.about.architecture.core": {"tr": "Veri işleme ve analiz mantığı için saf Python bilimsel çekirdek", "en": "Pure-Python scientific core for data processing and analysis logic"},
    "dash.about.architecture.workspace": {"tr": "Tekrarlanabilir karakterizasyon iş akışları için proje tabanlı çalışma alanı modeli", "en": "Project-based workspace model for reproducible characterization workflows"},
    "dash.about.architecture.note": {
        "tr": "Mimari, analiz mantığını kullanıcı arayüzünden bağımsız tutacak şekilde tasarlanmıştır; bu da masaüstü paketleme, bulut dağıtımı ve laboratuvar iş akışı entegrasyonlarına doğru gelecekteki genişlemeyi mümkün kılar.",
        "en": "The architecture is designed to keep analysis logic independent from the user interface, allowing future expansion toward desktop packaging, cloud deployment, and laboratory workflow integrations.",
    },
    "dash.about.status.title": {"tr": "Güncel Geliştirme Durumu", "en": "Current Development Status"},
    "dash.about.status.copy": {
        "tr": "MaterialScope şu anda aktif geliştirme aşamasındadır. Mevcut odak; Dash/Plotly kullanıcı deneyimini iyileştirmek, figür yakalama iş akışlarını standartlaştırmak, her analiz modülünü güçlendirmek ve platformu malzeme karakterizasyon kullanıcılarıyla uzman testlerine hazırlamaktır.",
        "en": "MaterialScope is currently in active development. The current focus is improving the Dash/Plotly user experience, standardizing figure capture workflows, strengthening each analysis module, and preparing the platform for expert testing with materials characterization users.",
    },
    "dash.about.notice.title": {"tr": "Araştırma / Doğrulama Notu", "en": "Research / Validation Notice"},
    "dash.about.notice.copy": {
        "tr": "Bu aşamada MaterialScope bir araştırma ve iş akışı destek aracı olarak tasarlanmıştır. Sertifikalı laboratuvar cihazlarının, doğrulanmış regülasyon yazılımlarının veya uzman yorumunun yerine geçmez.",
        "en": "At this stage, MaterialScope is intended as a research and workflow-support tool. It does not replace certified laboratory instruments, validated regulatory software, or expert interpretation.",
    },
    "sidebar.project": {
        "tr": "Proje",
        "en": "Project",
