...
def _literature_compare_card() -> dbc.Card:
    """Manual literature compare (same interaction model as DTA / TGA)."""
    return build_literature_compare_card(id_prefix="dsc")
            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        _dsc_result_section(build_figure_artifact_surface("dsc"), role="hero"),
...
def compare_dsc_literature(n_clicks, project_id, result_id, max_claims, persist_values, locale_data):
    ...
    return (
        render_literature_output(
            payload,
            loc,
            i18n_prefix=_DSC_LITERATURE_PREFIX,
...
@callback(
    Output("dsc-figure-save-snapshot-btn", "children"),
    Output("dsc-figure-use-report-btn", "children"),
    Output("dsc-figure-artifacts-summary", "children"),
    Input("ui-locale", "data"),
)
def render_dsc_figure_artifact_button_labels(locale_data):
    return figure_artifact_button_labels(_loc(locale_data))


@callback(
    Output("dsc-figure-save-snapshot-btn", "disabled"),
    Output("dsc-figure-use-report-btn", "disabled"),
    Input("dsc-latest-result-id", "data"),
)
def toggle_dsc_figure_artifact_buttons(result_id):
    disabled = not bool(result_id)
    return disabled, disabled


@callback(
    Output("dsc-result-figure-artifacts", "children"),
    Input("dsc-latest-result-id", "data"),
    Input("dsc-figure-artifact-refresh", "data"),
    Input("ui-locale", "data"),
    State("project-id", "data"),
)
def refresh_dsc_figure_artifacts_panel(result_id, _artifact_refresh, locale_data, project_id):
    loc = _loc(locale_data)
    if not result_id or not project_id:
        return ""
    ...
    previews = _dsc_fetch_figure_preview_data_urls(project_id, result_id, artifacts) if ordered_figure_preview_keys(artifacts) else None
    return build_figure_artifacts_panel(artifacts, loc, previews=previews)


@callback(
    Output("dsc-figure-artifact-status", "children"),
    Output("dsc-figure-artifact-refresh", "data"),
    Input("dsc-figure-save-snapshot-btn", "n_clicks"),
    Input("dsc-figure-use-report-btn", "n_clicks"),
    Input("dsc-latest-result-id", "data"),
    State("project-id", "data"),
    State("dsc-result-figure", "children"),
    State("ui-locale", "data"),
    State("dsc-figure-artifact-refresh", "data"),
    prevent_initial_call=True,
)
def dsc_figure_snapshot_or_report_figure(_snap_clicks, _report_clicks, latest_result_id, project_id, figure_children, locale_data, refresh_value):
    loc = _loc(locale_data)
    triggered_id = getattr(dash.callback_context, "triggered_id", None)
    if triggered_id == "dsc-latest-result-id":
        return "", dash.no_update
    action = figure_action_from_trigger(
        triggered_id,
        snapshot_button_id="dsc-figure-save-snapshot-btn",
        report_button_id="dsc-figure-use-report-btn",
    )
    if action is None:
        raise dash.exceptions.PreventUpdate
    if not project_id or not latest_result_id:
        return (
            figure_action_status_alert(loc, action=action, status="missing", reason="missing_project_or_result", class_prefix="dsc"),
            dash.no_update,
        )
    ...
    try:
        ...
        return (
            figure_action_status_alert(loc, action=action, status="error", reason=str(exc), class_prefix="dsc"),
            dash.no_update,
        )
    ...
    if outcome.get("status") == "ok":
        ...
        return (
            figure_action_status_alert(loc, action=action, status="ok", figure_key=key, class_prefix="dsc"),
            (refresh_value or 0) + 1,
        )
    if outcome.get("status") == "error":
        return (
            figure_action_status_alert(loc, action=action, status="error", reason=str(outcome.get("reason") or ""), class_prefix="dsc"),
            dash.no_update,
        )
    return (
        figure_action_status_alert(loc, action=action, status="skipped", reason=str(outcome.get("reason") or ""), class_prefix="dsc"),
        dash.no_update,
    )
