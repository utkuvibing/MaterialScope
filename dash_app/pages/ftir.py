...
_FTIR_LITERATURE_HINT_PREFIX = "dash.analysis.ftir.literature"
            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        build_literature_compare_card(id_prefix="ftir"),
                    ],
                    md=8,
                ),
            ]
...
def render_ftir_literature_chrome(locale_data, result_id):
    ...
    if result_id:
        hint = literature_t(
            loc,
            f"{_FTIR_LITERATURE_HINT_PREFIX}.ready",
            "Compare the saved FTIR result to literature sources.",
        )
    else:
        hint = literature_t(
            loc,
            f"{_FTIR_LITERATURE_HINT_PREFIX}.empty",
...
def compare_ftir_literature(n_clicks, project_id, result_id, max_claims, persist_values, locale_data):
    ...
    if not project_id or not result_id:
        msg = literature_t(
            loc,
            f"{_FTIR_LITERATURE_HINT_PREFIX}.missing_result",
