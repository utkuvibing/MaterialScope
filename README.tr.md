...
MaterialScope, GNU Affero Genel Kamu Lisansı sürüm 3 (yalnızca AGPL-3.0-only) altında lisanslanmıştır — ayrıntılar için [LICENSE](LICENSE) dosyasına bakın. AGPL koşullarına uyulduğu sürece ticari kullanıma izin verilir. AGPL'nin copyleft ve kaynak paylaşımı yükümlülüklerine tabi olmadan MaterialScope'u kullanmak, değiştirmek, dağıtmak, gömmek veya sunmak isteyen kuruluşlar, telif hakkı sahibinden ayrı bir mülkiyet/ticari lisans hakkında bilgi alabilir — bkz. [COMMERCIAL-LICENSING.md](COMMERCIAL-LICENSING.md). Lisans geçişinden önce yayımlanan sürümler MIT Lisansı altındaydı ve o sürümler için geçerli olan lisansla kullanılabilir olmaya devam eder — bkz. [LICENSE_HISTORY.md](LICENSE_HISTORY.md). MaterialScope adı ve logosu, resmî statü izlenimi veren kullanımlar için lisanslanmamıştır — bkz. [TRADEMARKS.md](TRADEMARKS.md).

Yazılımın telif lisansı, uygulamanın isteğe bağlı çalışma zamanı lisans anahtarı mekanizmasından (`MATERIALSCOPE_LICENSE_SECRET`, HMAC imzalı anahtarlar) ayrı bir kavramdır. Bu mekanizma yalnızca bir dağıtım özellik anahtarıdır: telif lisansını değiştirmez, onun yerine geçmez veya ona ekleme yapmaz; ortam değişkeninin ayarlanması yazılımı yeniden lisanslamaz.

İsteğe bağlı ticari lisans denetimi API anahtarından da ayrıdır. HMAC demo sırrı herkese açıktır; bu sırrı bilen herkes lisans anahtarı üretebilir. Ticari kurulumlarda `MATERIALSCOPE_LICENSE_SECRET` dışarıdan sağlanmalıdır; sırrın değiştirilmesi mevcut imzalı lisansları geçersiz kılar ve yeniden düzenlenmelerini gerektirir. İstemcide tutulan ortak bir sır, dağıtılan uygulamalar için yine de güvenli lisanslama sağlayamaz. Asimetrik imzalama sonraki bir çalışmadır.
