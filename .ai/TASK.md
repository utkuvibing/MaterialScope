...
## Status: active — XRD execution queue (ordered slices) + polish landed (2026-04-21)
...
7. **Slice 7 — XRD UX / hierarchy (2026-04-21):** Done — result surface: figure card + **inline toolbar** (overlay + snapshot/report); **figure artifacts** moved under collapsed `Details`; **plot settings** under **Plot appearance (advanced)** on Processing tab; **calmer defaults** in [`dash_app/components/xrd_processing_draft.py`](dash_app/components/xrd_processing_draft.py) / [`dash_app/components/xrd_result_plot.py`](dash_app/components/xrd_result_plot.py) (match overlays off, optional smoothed/baseline via `show_intermediate_traces`, leaner legend); **compact** candidate cards + evidence headings; **literature** [`build_literature_compare_card(..., compact_toolbar=True)`](dash_app/components/literature_compare_ui.py) + [`render_literature_output(..., collapse_retained_evidence=True)`](dash_app/components/literature_compare_ui.py); i18n + [`tests/test_xrd_dash_page.py`](tests/test_xrd_dash_page.py) / [`tests/test_xrd_processing_draft.py`](tests/test_xrd_processing_draft.py).
8. **Slice 8 — XRD final visual polish (2026-04-21):** Done — no new features: [`dash_app/pages/xrd.py`](dash_app/pages/xrd.py) + [`dash_app/assets/style.css`](dash_app/assets/style.css) (results rhythm, toolbar `ButtonGroup` + wrap, quieter artifacts/evidence bands, processing tab pane density, literature card chrome); [`utils/i18n.py`](utils/i18n.py) copy pass; [`tests/test_xrd_dash_page.py`](tests/test_xrd_dash_page.py) registry assertion uses `translate_ui`.

**Verification (current)**

- `rtk python -m py_compile backend/app.py backend/models.py backend/detail.py core/figure_preview_resize.py dash_app/components/analysis_page.py dash_app/components/literature_compare_ui.py dash_app/components/xrd_processing_draft.py dash_app/components/xrd_result_plot.py dash_app/pages/xrd.py dash_app/api_client.py utils/i18n.py tests/test_backend_figure_get.py tests/test_result_detail_figure_artifacts.py tests/test_xrd_dash_page.py`
...
- After polish: `rtk pytest -q tests/test_xrd_dash_page.py` (25 tests).
