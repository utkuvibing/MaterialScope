...
plan: 04
subsystem: api
tags: [xrd, compare, export, report, validation]
requires:
  - phase: 04-03
    provides: deterministic XRD qualitative candidate serialization with caution semantics
provides:
...
affects: [phase-signoff, compare-workspace, backend-exports, docx-reporting]
tech-stack:
  added: []
  patterns:
    - modality-adapter eligibility parity between backend compare contracts and Streamlit compare UI
    - caution-aware XRD reporting semantics for no-match and low-confidence qualitative outputs
key-files:
  created: []
  modified:
    - backend/detail.py
