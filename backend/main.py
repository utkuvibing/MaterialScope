...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MaterialScope backend service.")
...
def main() -> None:
    ...
    print(f"MaterialScope backend starting on {bind_url}", flush=True)
    if REPO_DOTENV_PATH.exists():
        print(f"MaterialScope backend env: {REPO_DOTENV_PATH}", flush=True)
    try:
    ...
    async def _log_bound_address() -> None:
        ...
        if bootstrap_status:
            ...
            print(
                "MaterialScope hosted library XRD coverage "
        ...
        if state == "published":
            print(
                "MaterialScope hosted library bootstrap published "
                f"{bootstrap_status.get('dataset_count', 0)} datasets from {bootstrap_status.get('source_root')}",
                flush=True,
            )
        elif state == "upgraded":
            print(
                f"MaterialScope hosted library upgraded: "
            ...
            print(
                f"MaterialScope hosted library already present "
            ...
            print(
                f"MaterialScope hosted library bootstrap failed: {bootstrap_status.get('message')}",
                flush=True,
            )
    ...
    try:
        ...
        raise SystemExit(f"MaterialScope backend failed to start on {bind_url}: {exc}") from exc
