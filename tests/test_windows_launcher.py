...
def _load_windows_launcher():
    ...
    spec = importlib.util.spec_from_file_location("materialscope_windows_launcher", launcher_path)
...
def test_ensure_user_runtime_creates_expected_directories_and_config(monkeypatch, tmp_path):
    ...
    assert not (user_root / ".materialscope_write_probe").exists()
    assert not (user_root / "support_logs" / ".materialscope_write_probe").exists()
    assert not (user_root / ".streamlit" / ".materialscope_write_probe").exists()
    assert launcher.os.environ["MATERIALSCOPE_HOME"] == str(user_root)
