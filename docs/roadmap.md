...
| WP | Scope | Status | Notes |
|---|---|---|---|
| **PR-8** | Sign-convention canon: one enum applied at ingest; labels derived post-inversion; resolves internal contradiction (`peak_analysis.py`) | ⬜ Planned (plan below) | Acceptance: both-convention golden test end-to-end. Evidence: four contradictory conventions coexist — `peak_analysis.find_thermal_peaks` hardcodes up=exotherm; `integrate_peak` docstring claims positive=endotherm; `ui/dsc_page.py` offers "up (endotherm)"; `processing_schema.py` declares display-only `dsc.endotherm_up` vs `dta.exotherm_up`. Canonical decision: exo-up |
| **PR-9** | Dimensional honesty: ΔCp either β-corrected or relabeled; peak area β-aware J/g path or honest relabeling | ⬜ Planned | Acceptance: known-β conversion golden test. Builds on the PR-8 canonical frame for area sign semantics |
...
**Execution record:** planned against main `dca974d` (PR #34 merged). Not yet implemented; the plan below is the work-package record and is retained after implementation as the execution record.
