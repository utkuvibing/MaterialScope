...
**Created:** 2026-08-26 · **Baseline:** `ee0325b` · **Current:** PR-9 merged as [PR #37](https://github.com/utkuvibing/MaterialScope/pull/37) (2026-09-07; main @ `32f41da`). Phase 1 planned as PR-8…PR-14, one PR per work package — all seven are now implemented: PR-8 and PR-9 merged; PR-10…PR-14 implemented, committed, and pushed on dedicated branches awaiting review/merge. There is no PR-15 work package; Phase 1 ends at PR-14.
...
| **PR-10** | Temperature-scale gate: K-vs-°C plausibility check at import (`validation.py`); unit propagated into reasoning prose | 🟡 Implemented — branch `pr-10-temperature-scale-gate` @ `4fd7ac1` (pushed; **not merged**) | Acceptance: K fixture blocked; °C fixture labeled correctly throughout reports. 16 gate tests + 123 related tests pass; ruff clean |
| **PR-11** | DTA characterization: reuse `characterize_peaks`; fix documented-kwarg crash; align docstring | 🟡 Implemented — branch `pr-11-dta-characterization` @ `8566d2b` (pushed; **not merged**) | `find_peaks` accepts documented kwargs (`min_width`, `rel_height`, `min_peak_height`), runs `characterize_peaks` itself (onset/endset/area/FWHM/height populated), direction-aware FWHM; stage-scoped kwarg routing in `process()`; redundant post-hoc pass removed from `batch_runner`. Full suite 1383 passed / 16 skipped; ruff clean |
| **PR-12** | Normalization & confidence guards: detect already-specific heat flow (opt-in re-normalization); fit-quality band defaults "low" without statistics | 🟡 Implemented — branch `pr-12-normalization-guards` @ `dbe6f2b` (pushed; **not merged**) | `resolve_working_unit` returns explicit skip reasons (`already_specific_power`, `sample_mass_missing`, …); re-normalization is opt-in via `normalize_force`; skip reason + source/working units recorded in processing provenance and batch summary; no-statistics confidence band defaults `"low"`; DSC page exposes normalization state honestly. PR-8 canonical frame untouched — detection is structural (unit class), not heuristic |
| **PR-13** | Import honesty pack: dropped-row/bad-line counts surfaced in metadata; ±Inf rejected at import; thousands-separator handling; encoding-mojibake warning; Excel sheet picker | 🟡 Implemented — branch `pr-13-import-honesty` @ `c325802` (pushed; **not merged**; stacked on `pr-10-temperature-scale-gate`) | Distinct NaN/Inf/bad-line/dropped counts in metadata + warnings; thousands-separator detection; UTF-8-fallback mojibake warning; `list_excel_sheets` + sheet picker wired wizard→API→backend; fabricated mass/heating-rate wizard defaults removed. PR-8 declaration surface reused, not reopened. 229 targeted tests pass; ruff clean |
| **PR-14** | Store hygiene: copy-on-read + per-project locks; disk autosave journal (crash-safe workspace restore) | 🟡 Implemented — branch `pr-14-store-hygiene` @ `16d8db3` (pushed; **not merged**) | `get`/`set` deep-copy isolation; per-project RLocks; atomic pickle journal (`<id>.msjournal`, temp-file + `os.replace`) with lazy restore after restart — lossless incl. live `ThermalDataset` objects; corrupt journals ignored. No science changes. 12 store tests + 113 backend tests pass; ruff clean |
...
3. ~~Fix P0-1…P0-5 scientific integrity pack~~ → Phase 1 correctness pack PR-8…PR-12; ~~PR-8 (sign-convention canon)~~ ✔ (PR #35); ~~PR-9 (dimensional honesty)~~ ✔ (PR #37); PR-10 (temperature-scale gate) + PR-11 (DTA characterization) + PR-12 (normalization/confidence guards) implemented on branches, merge pending
4. ~~Import honesty pack~~ → PR-13 implemented on `pr-13-import-honesty` (stacked on PR-10), merge pending
5. ~~pyproject + dependency pins (PR-3)~~ ✔ (PR #24)
6. ~~Security trio (PR-5)~~ ✔ (PR #31)
7. ~~Disk autosave + copy-on-read store~~ → PR-14 implemented on `pr-14-store-hygiene`, merge pending
...
| 2026-09-14 | PR-10 implemented on `pr-10-temperature-scale-gate` @ `4fd7ac1` from `32f41da`: declared-vs-defaulted temperature-unit extraction, K-vs-°C plausibility record at import enforced as a validation failure (API 400; Dash wizard gets an explicit scale-confirmation control), temperature unit threaded into reasoning prose. Pushed; merge pending |
| 2026-09-14 | PR-11 implemented on `pr-11-dta-characterization` @ `8566d2b` from `32f41da`: DTA `find_peaks` accepts the documented kwargs and characterizes peaks itself (onset/endset/area/height populated; direction-aware FWHM fixes the degenerate endo-peak FWHM in the exo-up canon); `process()` routes smoothing/baseline/peak kwargs per stage; `batch_runner` post-hoc characterization removed as redundant. Pushed; merge pending |
| 2026-09-14 | PR-12 implemented on `pr-12-normalization-guards` @ `dbe6f2b` from `32f41da`: structural already-specific-power detection with explicit skip reasons in provenance; opt-in `normalize_force` re-normalization; idempotent `normalize()`; no-statistics confidence band defaults `"low"`; DSC page surfaces normalization state + forced opt-in control. Pushed; merge pending |
| 2026-09-14 | PR-13 implemented on `pr-13-import-honesty` @ `c325802`, stacked on `pr-10-temperature-scale-gate` (shared import surface): distinct NaN/Inf/bad-line/dropped-row counts in metadata and warnings; ±Inf rejected; thousands-separator detection; UTF-8-fallback mojibake warning; `list_excel_sheets` + sheet picker wired wizard→API→backend; fabricated mass/heating-rate wizard defaults removed. Pushed; merge pending |
| 2026-09-14 | PR-14 implemented on `pr-14-store-hygiene` @ `16d8db3` from `32f41da`: copy-on-read/write deep-copy isolation, per-project RLocks, atomic `.msjournal` autosave journal (temp-file + `os.replace`) with lazy restore — lossless workspace recovery incl. live `ThermalDataset` objects that `.scopezip` cannot round-trip. No science changes. Pushed; merge pending |
| 2026-09-14 | **No PR-15 exists**: Phase 1 is defined as PR-8…PR-14 and no PR-15 work package appears in docs, planning dirs, git history, branches, or GitHub issues. Nothing was invented for it; if a fifteenth package is wanted it needs a defined scope first |
