...
from core.ftir_literature_query_builder import (
    _ftir_query_is_too_narrow,
...
FTIR_DISTRACTOR_TERMS = THERMAL_GENERIC_NEIGHBOR_TERMS | {
    "x-ray diffraction",
    "xrd",
    "chromatography",
    "hplc",
...
def _ftir_query_payload(record: Mapping[str, Any]) -> dict[str, Any]:
    return build_ftir_literature_query(record)


def _ftir_subject_tokens(query_payload: Mapping[str, Any], record: Mapping[str, Any]) -> set[str]:
    tokens: set[str] = set()
    summary = dict(record.get("summary") or {})
    metadata = dict(record.get("metadata") or {})
...
def _ftir_subject_label(query_payload: Mapping[str, Any], record: Mapping[str, Any]) -> str:
    summary = dict(record.get("summary") or {})
    metadata = dict(record.get("metadata") or {})
    return (
        _clean_text(query_payload.get("query_display_title"))
...
def _ftir_precision_signals(query_payload: Mapping[str, Any]) -> dict[str, Any]:
    snap = dict(query_payload.get("evidence_snapshot") or {})
    entity_terms = _dedupe(
        [
            _clean_text(query_payload.get("query_display_title")),
    ...
    modality_terms = list(FTIR_DIRECT_MODALITY_TERMS)
...
def _ftir_relevance_score(
    *,
    ...
    signals = _ftir_precision_signals(query_payload)
    entity_hits = _phrase_overlap_count(source_text, signals["entity_terms"])
    process_hits = _phrase_overlap_count(source_text, signals["process_terms"])
    modality_hits = _phrase_overlap_count(source_text, signals["modality_terms"])
    wn_hits = _phrase_overlap_count(source_text, signals["wavenumber_terms"])
    distractor_hits = _phrase_overlap_count(source_text, list(FTIR_DISTRACTOR_TERMS))
    direct_modality = _phrase_overlap_count(source_text, list(FTIR_DIRECT_MODALITY_TERMS))
...
def _ftir_comparison_note(
    *,
    ...
    if top_match and mstat == "matched":
        anchor = (
            f"The current FTIR ranking highlights {top_match} as a qualitative library target (not a confirmed chemical identification). "
        )
    ...
    if posture == "alternative_interpretation":
        return (
            f"{anchor}This paper discusses vibrational or infrared-related work relevant to {subject}, but it may favor an alternative reading "
            f"rather than supporting the present FTIR screening outcome. {basis_note}"
        )
    if posture == "related_support":
        pk = f"Detected peak budget in-record: {peak_n}." if peak_n is not None else ""
        return (
            f"{anchor}This paper discusses FTIR / infrared evidence relevant to {subject}. "
    ...
    if posture == "contextual_only":
        return (
            f"{anchor}This paper provides infrared or vibrational-spectroscopy context around {subject}, without tightly matching the retained peaks or ranking. {basis_note}"
        )
    return (
...
def _compare_ftir_result_to_literature(
    ...
    *,
    ...
    analysis_type = "FTIR"
    query_payload = _ftir_query_payload(record)
    ...
    query_presentation = build_ftir_query_presentation(query_payload)
    ...
    subject_tokens = _ftir_subject_tokens(query_payload, record)
    subject_label = _ftir_subject_label(query_payload, record)
    ...
    for source in search_results.values():
        ...
        hint = _clean_text((source.get("provenance") or {}).get("comparison_hint")).lower()
        precision_score, low_specificity, wavenumber_hits, entity_hits = _ftir_relevance_score(
            source_text=source_text,
            access_class=access_class,
            query_payload=query_payload,
            overlap=overlap,
        )
        modality_hits = _phrase_overlap_count(source_text, _ftir_precision_signals(query_payload)["modality_terms"])
        posture = _ftir_validation_posture(
            access_class=access_class,
            overlap=overlap,
            source_text=source_text,
            hint=hint,
        ...
        note = _ftir_comparison_note(
            subject=subject_label,
            posture=posture,
            query_payload=query_payload,
            access_field=access_field,
        )
        evidence_scope = _ftir_evidence_scope(
            query_payload=query_payload,
            source_text=source_text,
