...
def _build_raman_reasoning(
    ...
    claims.append(
        _claim(
            "C1",
            "descriptive",
            f"The RAMAN workflow executed spectral preprocessing, peak detection, and reference similarity matching, ending in match_status={match_status}.",
            desc_ev,
        )
    )
    evidence_map["C1"] = desc_ev
    ...
    return {
        ...
        "next_experiments": recommend_next_experiments("RAMAN", metadata_gaps=gaps, fit_band=fit_band),
    }
...
def build_scientific_reasoning(
    *,
    ...
    if analysis == "RAMAN":
        return _build_raman_reasoning(summary, rows, metadata, band, fit_reason, validation)
    if analysis == "XRD":
