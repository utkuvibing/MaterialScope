"""Deterministic RAMAN literature query payloads for spectral-similarity records."""
...
def build_raman_literature_query(record: Mapping[str, Any]) -> dict[str, Any]:
    """Build a traceable RAMAN literature query payload (spectral similarity / library context)."""
    ...
    signal_role = _clean_text(method_context.get("raman_signal_role") or method_context.get("ftir_signal_role")).lower()
    ...
    if match_status == "library_unavailable":
        ...
        prioritized.extend(
            [
                ...
                " ".join([modality_terms[1], "qualitative screening", "best practices"]),
            ]
        )
        rationale_parts.append(
            ...
            "asserting a ranked spectral identification and instead targets RAMAN methodology and library-matching practice."
        )
        ...
        if wn_terms:
            prioritized.append(" ".join([modality_terms[0], top_name, *wn_terms[:3], "raman bands"]))
        if subject_label:
            prioritized.append(" ".join([quoted_subject or subject_label, modality_terms[0], top_name, "raman"]))
        prioritized.append(" ".join([modality_terms[0], "spectral similarity", "library screening"]))
        rationale_parts.append(
            f"The literature search is anchored to the retained top spectral candidate ({top_name}) as a qualitative RAMAN library outcome"
        ...
        if subject_label and search_mode == "known_material":
            prioritized.append(
                " ".join(part for part in [quoted_subject or subject_label, modality_terms[0], modality_terms[2], "raman bands"] if part)
            )
        if wn_terms and peak_count >= 2:
            prioritized.append(" ".join([modality_terms[0], *wn_terms[:4], "raman shift"]))
        ...
        if subject_label:
            prioritized.append(" ".join([quoted_subject or subject_label, "raman spectroscopy", "vibrational modes"]))
        prioritized.append(" ".join([modality_terms[1], "spectral similarity"]))
        rationale_parts.append(
            "The RAMAN literature search uses modality-first wording aligned to the current spectral-similarity summary"
    ...
    display_title = subject_label or top_name or "RAMAN spectral screening"
    display_mode = "RAMAN / vibrational spectroscopy"
...
def build_raman_query_presentation(query_payload: Mapping[str, Any]) -> dict[str, Any]:
...
def _raman_query_is_too_narrow(query_payload: Mapping[str, Any]) -> bool:
